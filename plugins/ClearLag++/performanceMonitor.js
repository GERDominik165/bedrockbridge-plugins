...
import { world, system, Player, DimensionTypes, DisplaySlotId } from "@minecraft/server";
...
export class PerformanceMonitor {
  ...
  initialize() {
    ...
    try {
      ...
      console.warn("§e[ClearLag++]§r Scoreboard setup failed:", e.message);
    }
  ...
  /**
   * Richtet Scoreboard für live Stats ein (wie TPS.js)
   */
  setupScoreboard() {
    try {
      this.scoreBoard = world.scoreboard.getObjective("clearlag:stats") ??
        world.scoreboard.addObjective("clearlag:stats", "ClearLag++ Statistics");

      // Cleanup alte Einträge
      this.scoreBoard.getParticipants().forEach(p => {
        try {
          this.scoreBoard.removeParticipant(p);
        } catch (e) {}
      });
    } catch (error) {
      console.warn("§e[ClearLag++]§r Could not setup scoreboard:", error.message);
    }
  }

  /**
   * Updated Scoreboard mit aktuellen Statistiken
   */
  updateScoreboard() {
    ...
    try {
      ...
      this.scoreBoard.setScore("§bTPS", Math.floor(metrics.tps));
      this.scoreBoard.setScore("§7MSPT", metrics.mspt);
      this.scoreBoard.setScore("§aEntities", metrics.entityCount);
      this.scoreBoard.setScore("§6Items", metrics.itemCount);
      this.scoreBoard.setScore("§cMobs", metrics.mobCount);
    } catch (e) {
      // Silent fail for scoreboard
    }
  }
  ...
  /**
   * Zählt Entities in allen Dimensionen mit Filter (wie TPS.js)
   */
  ...
  updateMetrics() {
    ...
    // Entity Counts - Effiziente Filterung wie in TPS.js
    try {
      // Zähle alle Entities
      this.metrics.entityCount = this.getEntityCountByFilter({});

      // Zähle Items
      this.metrics.itemCount = this.getEntityCountByFilter({ type: "minecraft:item" });

      // Zähle Spieler
      this.metrics.playerCount = world.getAllPlayers().length;

      // Zähle Mobs (alles außer Items und Spieler)
      this.metrics.mobCount = this.getEntityCountByFilter({
        excludeTypes: ["minecraft:item", "minecraft:player"]
      });
    } catch (error) {
      // Silent fallback - verwende alte Werte
    }
