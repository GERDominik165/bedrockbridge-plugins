...
export class CommandHandler {
  ...
  /**
   * Registriert alle Commands mit Bedrock Bridge (wie adminTPmenu.js)
   */
  registerCommands(bridge) {
    ...
    // Registriere Haupt-Admin Commands
    try {
      ...
      console.log("§a[ClearLag++]§r Haupt-Command 'clearlag' registriert!");
    } catch (error) {
      console.warn("§e[ClearLag++]§r Fehler beim Registrieren des Haupt-Commands:", error.message);
    }

    // Registriere Sub-Commands als Admin Commands
    for (const [name, command] of Object.entries(this.commands)) {
      try {
        if (command.permission === "admin") {
          bridge.bedrockCommands.registerAdminCommand(
            ...
            (player) => {
              if (this.hasPermission(player, command.permission)) {
                ...
                this.sendError(player, "Keine Berechtigung für diesen Command!");
              }
            },
            command.description
          );
          console.log(`§a[ClearLag++]§r Admin-Command 'clearlag ${name}' registriert!`);
        } else {
          ...
          console.log(`§a[ClearLag++]§r Public-Command 'clearlag ${name}' registriert!`);
        }
      } catch (error) {
        console.warn(`§e[ClearLag++]§r Fehler beim Registrieren von 'clearlag ${name}':`, error.message);
      }
    }
  ...
  /**
   * Haupt-Command Handler
   */
  handleMainCommand(player) {
    if (!player || !player.isValid) return;

    if (!this.hasPermission(player, "user")) {
      this.sendError(player, "Keine Berechtigung!");
      return;
    }
