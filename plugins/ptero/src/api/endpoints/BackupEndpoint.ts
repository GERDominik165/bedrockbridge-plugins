...
import { IBackup, IBackupListResponse, ISignedUrl } from '../../types';
...
export class BackupEndpoint {
  ...
  ): Promise<IBackupListResponse> {
    const cacheKey = `server:${serverId}:backups:${page}:${perPage}`;

    if (useCache) {
      const cached = cacheManager.get<IBackupListResponse>(cacheKey);
   ...
   * Get backup details
   */
  async getBackup(serverId: string, backupId: string, useCache: boolean = true): Promise<IBackup> {
    const cacheKey = `server:${serverId}:backup:${backupId}`;

    if (useCache) {
      const cached = cacheManager.get<IBackup>(cacheKey);
      if (cached) return cached;
    }
   ...
   */
  async getBackupDownloadUrl(serverId: string, backupId: string): Promise<string> {
   ...
   * Download backup
   */
  async downloadBackup(serverId: string, backupId: string): Promise<any> {
   ...
   * Restore backup
   */
  async restoreBackup(serverId: string, backupId: string, truncate: boolean = true): Promise<void> {
   ...
   */
  async toggleBackupLock(serverId: string, backupId: string): Promise<IBackup> {
   ...
   * Lock backup
   */
  async lockBackup(serverId: string, backupId: string): Promise<IBackup> {
   ...
   * Unlock backup
   */
  async unlockBackup(serverId: string, backupId: string): Promise<IBackup> {
    const backup = await this.getBackup(serverId, backupId, false);
    if (backup.attributes.is_locked) {
      return await this.toggleBackupLock(serverId, backupId);
    }
