...
import { COMMAND_PREFIX, Colors, Icons, SUCCESS_MESSAGES } from './config/Constants';
...
export class PterodactylPlugin {
    ...
    async showMainMenu(player) {
        const form = new PterodactylActionForm(`${Icons.SERVER} Pterodactyl Bridge`);
     ...
     * Show server details
     */
    async showServerDetails(player, serverId) {
        try {
            ...
            const body = `${Colors.BOLD}Status:${Colors.RESET} ${attrs.status}\n` +
     ...
     * Start server
     */
    async startServer(player, serverId) {
        try {
            const form = FormBuilder.createConfirmDialog('Server starten?', 'Der Server wird gestartet.');
     ...
     * Stop server
     */
    async stopServer(player, serverId) {
        try {
            const form = FormBuilder.createConfirmDialog('Server stoppen?', 'Der Server wird gestoppt.');
            const confirmed = await form.show(player);
            if (!confirmed)
                return;
     ...
     * Restart server
     */
    async restartServer(player, serverId) {
        try {
            const form = FormBuilder.createConfirmDialog('Server neustarten?', 'Der Server wird neugestartet.');
            const confirmed = await form.show(player);
            if (!confirmed)
                return;
     ...
     * Open console
     */
    async openConsole(player, serverId) {
        try {
     ...
     * Show databases
     */
    async showDatabases(player, serverId) {
        try {
     ...
     * Show backups
     */
    async showBackups(player, serverId) {
        try {
     ...
     * Show files
     */
    async showFiles(player, serverId, directory = '/') {
        try {
     ...
     * Show server status
     */
    async showServerStatus(player, serverId) {
        try {
            const resources = await this.serverEndpoint.getResources(serverId);
            const res = resources.attributes;
            const statusText = `${Colors.BOLD}Status: ${Colors.RESET}${res.current_state}\n` +
                FormBuilder.formatResourceInfo(res.resources.cpu_absolute, res.resources.memory_bytes, res.resources.memory_limit_bytes, res.resources.disk_bytes);
     ...
     */
    async showMonitoringDashboard(player) {
     ...
     */
    showHelp(player) {
        const help = `${Colors.BOLD}${Icons.INFO} Pterodactyl Bridge - Hilfe${Colors.RESET}\n\n` +
     ...
     */
    showInfo(player) {
        const info = `${Colors.BOLD}${Icons.INFO} Pterodactyl Bridge - Info${Colors.RESET}\n\n` +
            `Version: 1.0.0\n` +
     ...
     * Helper methods
     */
    async showServerListForDatabases(player) {
    ...
    async showServerListForBackups(player) {
        const response = await this.serverEndpoint.listServers();
        const servers = response.data || [];
        if (servers.length === 0) {
            player.sendMessage(`${FormBuilder.formatInfoMessage('Keine Server gefunden')}`);
    ...
    async showServerListForFiles(player) {
        const response = await this.serverEndpoint.listServers();
        const servers = response.data || [];
        if (servers.length === 0) {
            player.sendMessage(`${FormBuilder.formatInfoMessage('Keine Server gefunden')}`);
    ...
    async rotateDatabasePassword(player, serverId, databaseId) {
        const confirmed = await FormBuilder.createConfirmDialog('Passwort erneuern?', 'Dies wird das Datenbankpasswort ändern.').show(player);
        if (!confirmed)
            return;
        try {
    ...
    async deleteDatabase(player, serverId, databaseId) {
        const confirmed = await FormBuilder.createConfirmDialog('Datenbank löschen?', 'Dies wird die Datenbank permanent löschen!').show(player);
        if (!confirmed)
            return;
        try {
    ...
    async createBackup(player, serverId) {
...
export let pterodactylPlugin;
// Plugin initialization hook
export async function initializePlugin(config) {
